...
function () {
    ...
    var TRUE = true,
        ...
        ENERGIZER_POSITIONS = [
            {x: 2, y: 2},   // oben links
            {x: 2, y: 16},  // unten links
            {x: 57, y: 2},  // oben rechts
            {x: 57, y: 16}  // unten rechts
        ],
        TUNNEL_POSITIONS = [
            {x: 1, y: 9},   // left T-tile: col=0, row=8
            {x: 58, y: 9}   // right T-tile: col=57, row=8
        ],
        STARTING_POSITIONS = {
            1: [
                {x: 37, y: 16, dir: 4},                                    // Pac-Man
                {x: 25, y: 2, dir: 8, scatterX: 57, scatterY: -4},         // Blinky (open corridor above pen)
                {x: 25, y: 5, dir: 2, scatterX: 1, scatterY: -4},          // Pinky (pen center)
                {x: 23, y: 5, dir: 1, scatterX: 57, scatterY: 20},         // Inky (pen left-center)
                {x: 27, y: 5, dir: 1, scatterX: 1, scatterY: 20}           // Clyde (pen right)
            ],
            2: [
                {x: 37.5, y: 16, dir: 8},  // Pac-Man A
                {x: 36.5, y: 16, dir: 4},  // Pac-Man B
                {x: 25, y: 2, dir: 8, scatterX: 57, scatterY: -4},
                {x: 25, y: 5, dir: 2, scatterX: 1, scatterY: -4},
                {x: 23, y: 5, dir: 1, scatterX: 57, scatterY: 20},
                {x: 27, y: 5, dir: 1, scatterX: 1, scatterY: 20}
            ]
        },
        PEN_ENTRANCE_POS = [24, 200],  // game(y=3, x=25) = Exit-Korridor oben
        FRUIT_POS = [88, 240],
        TIMING_VALUES = {
            0: 0.16,
            1: 0.23,
        ...
        // ============================================================
        GHOST_ROUTINES = {
            // Bobbing im Pen: rechteckiges Ghost House (y=4-6, x=22-27)
            // Inky (x=23, links-mitte), Pinky (x=25, mitte), Clyde (x=27, rechts)
            1: [{x: 23, y: 5, dir: 1, dest: 4.375, speed: 0.48},
                {x: 23, y: 4.375, dir: 2, dest: 5.625, speed: 0.48},
                {x: 23, y: 5.625, dir: 1, dest: 5, speed: 0.48}],
            2: [{x: 25, y: 5, dir: 2, dest: 5.625, speed: 0.48},
                {x: 25, y: 5.625, dir: 1, dest: 4.375, speed: 0.48},
                {x: 25, y: 4.375, dir: 2, dest: 5, speed: 0.48}],
            3: [{x: 27, y: 5, dir: 1, dest: 4.375, speed: 0.48},
                {x: 27, y: 4.375, dir: 2, dest: 5.625, speed: 0.48},
                {x: 27, y: 5.625, dir: 1, dest: 5, speed: 0.48}],
            // Exit: nach x=25 bewegen, dann hoch zu y=3 (Exit-Korridor oben)
            4: [{x: 23, y: 5, dir: 8, dest: 25, speed: PEN_EXIT_SPEED},
                {x: 25, y: 5, dir: 1, dest: 3, speed: PEN_EXIT_SPEED}],
            5: [{x: 25, y: 5, dir: 1, dest: 3, speed: PEN_EXIT_SPEED}],
            6: [{x: 27, y: 5, dir: 4, dest: 25, speed: PEN_EXIT_SPEED},
                {x: 25, y: 5, dir: 1, dest: 3, speed: PEN_EXIT_SPEED}],
            // Return: von y=3 (PEN_ENTRANCE_POS) nach unten zur Bob-Position
            7: [{x: 25, y: 3, dir: 2, dest: 5, speed: 1.6},
                {x: 25, y: 5, dir: 4, dest: 23, speed: 1.6}],
            8: [{x: 25, y: 3, dir: 2, dest: 5, speed: 1.6}],
            9: [{x: 25, y: 3, dir: 2, dest: 5, speed: 1.6},
                {x: 25, y: 5, dir: 8, dest: 27, speed: 1.6}],
            // Re-Exit (identisch mit Exit)
            10: [{x: 23, y: 5, dir: 8, dest: 25, speed: PEN_EXIT_SPEED},
                 {x: 25, y: 5, dir: 1, dest: 3, speed: PEN_EXIT_SPEED}],
            11: [{x: 25, y: 5, dir: 1, dest: 3, speed: PEN_EXIT_SPEED}],
            12: [{x: 27, y: 5, dir: 4, dest: 25, speed: PEN_EXIT_SPEED},
                 {x: 25, y: 5, dir: 1, dest: 3, speed: PEN_EXIT_SPEED}]
        },
        // ============================================================
    ...
    Actor.prototype.reset = function () {
        ...
        this.setSpeedType(0);
        this.reverseDirectionsNext = this.freeToLeavePen = this.modeChangedWhileInPen = this.eatenInThisFrightMode = this.permanentlyEaten = FALSE;
    ...
    Actor.prototype.decideNextDirection = function (b) {
        ...
        case 8:
            ...
            if (bfsDir) this.nextDir = bfsDir;
            break;
        case 2:
        case 1:
            var bfsDir12 = game.findModeEightDirection(c, this.targetPos, FALSE);
            if (bfsDir12) this.nextDir = bfsDir12;
            break;
        case 4:
            if ((this.dir & h.allowedDir) == 0 && h.allowedDir == game.oppositeDirections[this.dir]) {
                this.nextDir = game.oppositeDirections[this.dir];
            } else {
                var oppDir4 = game.oppositeDirections[this.dir];
                var validDirs4 = [], validNoGH4 = [];
                for (var fi4 = 0; fi4 < DIRECTIONS.length; fi4++) {
                    var fd4 = DIRECTIONS[fi4];
                    if ((fd4 & h.allowedDir) == 0 || fd4 == oppDir4) continue;
                    var fdDelta4 = DIRECTION_DELTAS[fd4];
                    var fdNx4 = [iPx[0], iPx[1]]; fdNx4[fdDelta4.axis] += fdDelta4.increment * 8;
                    validDirs4.push(fd4);
                    if (!(game.playfield[fdNx4[0]] && game.playfield[fdNx4[0]][fdNx4[1]] && game.playfield[fdNx4[0]][fdNx4[1]].ghostHouse))
                        validNoGH4.push(fd4);
                }
                var pool4 = validNoGH4.length > 0 ? validNoGH4 : validDirs4;
                this.nextDir = pool4.length > 0 ? pool4[Math.floor(game.rand() * pool4.length)] : oppDir4;
            }
            break
        }
    };
    ...
    Actor.prototype.handleIntersection = function () {
        ...
        if (b.intersection) if (this.nextDir && this.nextDir & b.allowedDir) {
            ...
            if (this.ghost) {
                // Geist-Rettung: Gegenrichtung oder nächste gültige Richtung wählen
                // (verhindert dauerhaftes Einfrieren im Ghosthouse oder an Sackgassen)
                var rescueDir = game.oppositeDirections[this.lastActiveDir];
                if (!rescueDir || !(rescueDir & b.allowedDir)) {
                    for (var ri = 0; ri < DIRECTIONS.length; ri++) {
                        if (DIRECTIONS[ri] & b.allowedDir) { rescueDir = DIRECTIONS[ri]; break; }
                    }
                }
                if (rescueDir) this.dir = rescueDir;
            }
            this.setSpeedType(0)
        }
    };
    ...
    Actor.prototype.advanceToNextRoutineMove = function () {
        this.routineMoveId++;
        if (this.routineMoveId == GHOST_ROUTINES[this.routineToFollow].length) if (this.mode == 16 && this.freeToLeavePen && !game.ghostExitingPenNow && !this.permanentlyEaten) {
            ...
            if (game.permanentFright && !this.permanentlyEaten) {
                this.permanentlyEaten = TRUE;
                this.el.style.display = 'none';
                this.freeToLeavePen = FALSE;
                this.setMode(16);
                game.ghostsPermanentlyEaten++;
                if (game.ghostsPermanentlyEaten >= 4) game.showWinPopup();
            } else if (this.id == game.playerCount || this.freeToLeavePen) {
                this.setMode(128);
            } else {
    ...
    game.prepareElement = function (b, c, d) {
        ...
        if (game.useCss) {
            b.style.backgroundImage = "url(eatbeerpacman10-hp-sprite.png)";
            b.style.backgroundPosition = -c + "px " + -d + "px";
            b.style.backgroundRepeat = "no-repeat"
        } else {
            b.style.overflow = "hidden";
            c = "display: block; position: relative; left: " + -c + "px; top: " + -d + "px";
            b.innerHTML = '<img style="' + c + '" src="eatbeerpacman10-hp-sprite.png">'
        }
    };
    ...
    // ============================================================
    // MAZE_MAP → PATH_SEGMENTS + DOT_FREE_ZONES builder
    // ============================================================
    game.buildFromMazeMap = function () {
        PATH_SEGMENTS = [];
        DOT_FREE_ZONES = [];
        var rows = MAZE_MAP.length, cols = MAZE_MAP[0].length;
        ...
        for (var r = 0; r < rows; r++) {
            ...
            while (c < cols) {
                var ch = MAZE_MAP[r][c];
                if (ch === '.' || ch === 'G' || ch === 'T') {
                    var start = c, firstCh = ch;
                    while (c < cols) {
                        var cur = MAZE_MAP[r][c];
                        if (cur !== '.' && cur !== 'G' && cur !== 'T') break;
                        c++;
                    }
                    var seg = {x: start + 1, y: r + 1, w: c - start};
                    if (firstCh === 'T') seg.type = 1;
                    PATH_SEGMENTS.push(seg);
                } else { c++; }
            }
        }

        // Vertical segments (length >= 2 only, to avoid duplicate single-tile coverage)
        for (var col = 0; col < cols; col++) {
            var r = 0;
            while (r < rows) {
                var ch = MAZE_MAP[r][col];
                if (ch === '.' || ch === 'G' || ch === 'T') {
                    var start = r;
                    while (r < rows) {
                        var cur = MAZE_MAP[r][col];
                        if (cur !== '.' && cur !== 'G' && cur !== 'T') break;
                        r++;
                    }
                    if (r - start >= 2)
                        PATH_SEGMENTS.push({x: col + 1, y: start + 1, h: r - start});
                } else { r++; }
            }
        }

        // DOT_FREE_ZONES: all G-tile runs (ghost house) + T-tile runs (off-screen tunnel)
        for (var r = 0; r < rows; r++) {
            var c = 0;
            while (c < cols) {
                if (MAZE_MAP[r][c] === 'G') {
                    var start = c;
                    while (c < cols && MAZE_MAP[r][c] === 'G') c++;
                    DOT_FREE_ZONES.push({x: start + 1, y: r + 1, w: c - start});
                } else if (MAZE_MAP[r][c] === 'T') {
                    var start = c;
                    while (c < cols && MAZE_MAP[r][c] === 'T') c++;
                    DOT_FREE_ZONES.push({x: start + 1, y: r + 1, w: c - start});
                } else { c++; }
            }
        }
    ...
    game.preparePaths = function () {
        ...
        // Mark ghost house tiles — prevents normal-mode ghosts from navigating into them
        for (var gRow = 0; gRow < MAZE_MAP.length; gRow++)
            for (var gCol = 0; gCol < MAZE_MAP[gRow].length; gCol++)
                if (MAZE_MAP[gRow][gCol] === 'G')
                    game.playfield[(gRow + 1) * 8][(gCol + 1) * 8].ghostHouse = TRUE;
    };
    ...
    game.findModeEightDirection = function (pos, targetPos, allowGhostHouse) {
        var queue = [{r: pos[0], c: pos[1], dir: 0}];
        var visited = {};
        ...
        while (queue.length > 0) {
            ...
            if (curr.r === targetPos[0] && curr.c === targetPos[1]) return curr.dir;
            for (var i = 0; i < dirDeltas.length; i++) {
                var dd = dirDeltas[i];
                var nr = curr.r + dd.dr, nc = curr.c + dd.dc;
                var key = nr + ',' + nc;
                if (!visited[key] && game.playfield[nr] && game.playfield[nr][nc] &&
                    (game.playfield[nr][nc].path || (allowGhostHouse && game.playfield[nr][nc].ghostHouse))) {
                    visited[key] = TRUE;
                    queue.push({r: nr, c: nc, dir: curr.dir || dd.dir});
                }
            }
        }
        return 0;
    ...
    game.createEnergizerElements = function () {
        ...
        for (var b in ENERGIZER_POSITIONS) {
            ...
            el.className = "pcm-e";
            el.textContent = ebIcons[parseInt(b, 10)] || '\uD83C\uDF7A';
    ...
    game.restartGameplay = function (b) {
        ...
        game.energizersEatenCount = 0;
        game.permanentFright = FALSE;
        game.ghostsPermanentlyEaten = 0;
    ...
    game.newGame = function () {
        if (game.canvasEl) game.canvasEl.style.display = '';
    ...
    game.createKillScreenElement = function (b, c, d, f, h) {
        ...
        j.style.zIndex = 119;
        if (h) {
            j.style.background = "url(eatbeerpacman10-hp-sprite.png) -" + game.killScreenTileX + "px -" + game.killScreenTileY + "px no-repeat";
    ...
    game.dotEaten = function (b, c) {
        ...
        var isEnergizer = game.playfield[c[0]][c[1]].dot == 2;
        if (isEnergizer) {
            ...
            if (window.ebHookEnergizerEaten) {
                ...
                for (var ei = 0; ei < ENERGIZER_POSITIONS.length; ei++) {
                    if (ENERGIZER_POSITIONS[ei].y * 8 === c[0] && ENERGIZER_POSITIONS[ei].x * 8 === c[1]) {
                        energizerIdx = ei;
                        break;
                    }
                }
                ...
                window.ebHookEnergizerEaten(energizerIdx, function () {
        ...
        if (!isEnergizer || !window.ebHookEnergizerEaten) {
            game.playAmbientSound()
        }
    };
    ...
    game.eatFruit = function (b) {
        if (game.fruitShown) {
            ...
            game.addToScore(game.levels.fruitScore + 200, b);
        }
    };
    ...
    game.playerDies = function (b) {
        ...
        if (window.ebHookPlayerDied) {
            var killerGhostId = -1;
            for (var gi = game.playerCount; gi < game.playerCount + 4; gi++) {
                if (game.actors[gi].tilePos[0] == game.actors[b].tilePos[0] &&
                    game.actors[gi].tilePos[1] == game.actors[b].tilePos[1] &&
                    !(game.actors[gi].mode & (8|16|32|64|128))) {
                    killerGhostId = gi - game.playerCount;
                    break;
                }
            }
            if (killerGhostId >= 0) {
    ...
    game.detectCollisions = function () {
        ...
        for (var b = game.playerCount; b < game.playerCount + 4; b++) for (var c = 0; c < game.playerCount; c++) if (game.actors[b].tilePos[0] == game.actors[c].tilePos[0] && game.actors[b].tilePos[1] == game.actors[c].tilePos[1]) if (game.actors[b].mode == 4) {
            game.ghostDies(b, c);
            return
        } else game.actors[b].mode != 8 && game.actors[b].mode != 16 && game.actors[b].mode != 32 && game.actors[b].mode != 128 && game.actors[b].mode != 64 && !game.actors[b].permanentlyEaten && game.playerDies(c)
    };
    ...
    game.finishFrightMode = function () {
        if (game.permanentFright) {
            game.frightModeTime = game.levels.frightTotalTime;
            return;
        }
        game.switchMainGhostMode(game.lastMainGhostMode, FALSE)
    };
    game.activatePermanentFright = function () {
        game.permanentFright = TRUE;
        game.switchMainGhostMode(4, FALSE);
    };
    game.showWinPopup = function () {
        window.clearInterval(game.tickTimer);
        game.tickTimer = null;
        if (game.soundAvailable) {
            try { game.flashSoundPlayer.stopAmbientTrack(); } catch(e) {}
        }
        document.getElementById('eb-popup-icon').innerHTML = '&#127881;';
        document.getElementById('eb-popup-title').textContent = 'Congratulations!';
        document.getElementById('eb-popup-animation').innerHTML = '';
        document.getElementById('eb-popup-text').textContent = 'You\u2019ve overcome the challenges and used your opportunities! Remember: \u201cDon\u2019t just drink beer. Eat it.\u201d';
        ...
        if (game.gameplayModeTime) {
            ...
            if (game.gameplayModeTime <= 0) {
                ...
                case 1:
                    ...
                    if (game.permanentFright) {
                        var pg = game.actors[game.ghostBeingEatenId];
                        pg.permanentlyEaten = TRUE;
                        pg.el.style.display = 'none';
                        pg.freeToLeavePen = FALSE;
                        pg.setMode(16);
                        game.ghostsPermanentlyEaten++;
                        if (game.ghostsPermanentlyEaten >= 4) { game.showWinPopup(); break; }
                    } else {
                        game.ghostEyesCount++;
    ...
    game.playDotEatingSound = function (b) {
        if (game.soundAvailable && google.pacManSound) {
            if (game.gameplayMode == 0 && b == 0) {
                try { game.flashSoundPlayer.playTrack('gluck', 4); } catch(e) {}
            }
        }
    ...
    game.everythingIsReady = function () {
        if (!game.ready) {
            ...
            google.dom.remove(b);
            // Banner bleibt sichtbar bis das Spiel startet (kein background='black' hier)
            game.addCss();
            ...
            // Tick-Timer NICHT starten bevor newGame() läuft – verhindert Fehler
            // durch fehlende Actors/Playfield-Elemente und sichert Sound-Initialisierung
            game.gameplayMode = 8;
            game.canvasEl.style.display = 'none'
        }
    };
    ...
    game.prepareGraphics = function () {
        ...
        game.preloadImage("eatbeerpacman10-hp-sprite.png")
    };
...
game.init = function () {
    ...
    // Sichere Version – prüft, ob Elemente existieren
